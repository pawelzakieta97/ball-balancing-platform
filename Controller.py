...
class Controller:
    def __init__(self, Tp=0.025, samples_num=5, poly=2, prevSamples=3, kind="PID_1"):
        ...
        self.poly = poly  # this is the highest power of the polynomial (on default 2- we are fitting a parabola)
        ...
        #current values of platform angles
        self.alfa = 0
        self.beta = 0
    ...
    def update(self, position, timeOverride=-10):
        ...
        if self.samples["x"].size > self.gpc["x"].model.num.size:
            ...
            self.virtualSamples["x"] = np.array(
                [polyValue(self.parameters["x"], float(now) / 1000 - (self.gpc["x"].model.den.size - i - 1) * self.Tp)
                 for i in range(0, self.gpc["x"].model.den.size)])
            self.virtualSamples["y"] = np.array(
                [polyValue(self.parameters["y"], float(now) / 1000 - (self.gpc["y"].model.den.size - i - 1) * self.Tp)
                 for i in range(0, self.gpc["y"].model.den.size)])
            ...
            if self.kind == "PID_1":
                ...
                ax = self.pid["x"].update(self.virtualSamples["x"][-1], self.setPoint["x"][0], derOverride=derValue,
                                          der2Override=der2Value)
        ...
        if self.steering_enable:
            [self.alfa, self.beta] = self.servos.applySteering(ax, ay)
